...
class DiscordOAuth2Service:
    ...
    def get_redirect_uri(cls):
        ...
        # Use SITE_URL if available, otherwise construct from request
        site_url = getattr(settings, 'SITE_URL', None)
        if site_url:
            return f"{site_url}/api/auth/discord/callback/"
        return "http://localhost:8000/api/auth/discord/callback/"
